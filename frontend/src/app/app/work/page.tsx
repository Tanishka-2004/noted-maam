      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#CDBEA9] pb-6">
          <div>
            <div className="flex items-center gap-2 text-xs font-mono text-[#681F32] font-bold mb-1">
              <CheckSquare className="w-4 h-4 text-[#681F32]" />
              <span>Work Workspace — Accountable Action Center ({mode === "DEMO" ? "Demo Mode" : "Real-World Mode"})</span>
            </div>
            <h1 className="text-3xl font-extrabold text-[#4A1724] tracking-tight">Work</h1>
            <p className="text-xs text-[#96546A] mt-1 font-medium">Answers <strong>"What do I need to do?"</strong> with strict human verification gates.</p>
          </div>
          ...
          <div className="flex items-center gap-1.5 bg-[#E9DFCE] p-1 rounded-xl border border-[#CDBEA9]">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${
                activeTab === "verification" ? "bg-[#681F32] text-[#FBF8F1]" : "text-[#4A1724] hover:bg-[#F7F1E5]"
              }`}
            >
              <ShieldCheck className="w-3.5 h-3.5 text-[#FBF8F1]" />
              <span>Verification Queue ({verificationItems.length})</span>
            </button>
            <button
              onClick={() => setActiveTab("my-tasks")}
              className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                activeTab === "my-tasks" ? "bg-[#681F32] text-[#FBF8F1]" : "text-[#4A1724] hover:bg-[#F7F1E5]"
              }`}
            >
              My Tasks
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${
                activeTab === "conflicts" ? "bg-[#681F32] text-[#FBF8F1]" : "text-[#4A1724] hover:bg-[#F7F1E5]"
              }`}
            >
              <AlertOctagon className="w-3.5 h-3.5 text-[#FBF8F1]" />
        ...
        {activeTab === "verification" && (
          <div className="space-y-4">
            <div className="p-4 rounded-xl bg-[#F7F1E5] border border-[#CDBEA9] flex items-center justify-between text-xs text-[#4A1724]">
            ...
            {mode === "REAL" && verificationItems.length === 0 ? (
              <div className="p-12 rounded-2xl bg-white border border-[#CDBEA9] text-center space-y-4">
                <CheckSquare className="w-12 h-12 text-[#681F32] mx-auto opacity-80" />
                <h3 className="text-lg font-bold text-[#4A1724]">No Unverified Commitments in Live Database</h3>
                <p className="text-xs text-[#96546A] max-w-md mx-auto">
                  Your live FastAPI backend has no pending action items for human verification. Upload a meeting recording or stream audio to extract commitments.
                </p>
                <div className="pt-2">
                  <Link
                    href="/onboarding"
                  ...
                  >
                    ...
                    <span>Upload Recording to Extract Tasks</span>
                  </Link>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                {verificationItems.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="p-6 rounded-2xl bg-[#F7F1E5] border border-[#CDBEA9] space-y-4 hover:border-[#7A2940] transition-all">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-mono text-[#4A1724] border border-[#CDBEA9] px-2 py-0.5 rounded bg-[#E9DFCE]">{item.project}</span>
                          </div>
                          <h3 className="text-base font-bold text-[#4A1724]">{item.title}</h3>
                          <div className="text-xs text-[#96546A] font-mono">
                            Assignee: <strong className="text-[#4A1724]">{item.owner}</strong> • Deadline: {item.deadline}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            className="px-3.5 py-2 rounded-xl border border-[#CDBEA9] bg-[#E9DFCE] hover:bg-[#CDBEA9] text-xs font-semibold text-[#4A1724] flex items-center gap-1.5"
                          >
                      ...
                      <div className="p-3 rounded-xl bg-[#FBF8F1] border border-[#CDBEA9] text-xs text-[#4A1724] flex items-center justify-between">
                        <div className="italic truncate max-w-2xl">"{item.provenance.quote}"</div>
                        <span className="text-[10px] font-mono text-[#681F32] font-bold shrink-0">{item.provenance.timeRange}</span>
                      </div>
                    </div>
                  );
                })}
        ...
        {activeTab === "my-tasks" && (
          <div className="p-8 rounded-2xl bg-[#F7F1E5] border border-[#CDBEA9] text-center space-y-3">
            <CheckSquare className="w-8 h-8 text-[#681F32] mx-auto" />
            <h3 className="font-bold text-[#4A1724] text-sm">
              {mode === "DEMO" ? "Your Personal Confirmed Commitments (Demo)" : "Real Workspace Tasks"}
            </h3>
            <p className="text-xs text-[#96546A] max-w-md mx-auto">
              {mode === "DEMO" 
              ...
              <>
                <div className="flex items-center justify-between border-b border-[#CDBEA9] pb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-base font-bold text-[#4A1724]">Target Launch Window vs Security Compliance</h2>
                  </div>
                  <Link href="/app/knowledge?tab=decisions" className="text-xs font-mono text-[#681F32] font-bold hover:underline">
                    Resolve via Decision Timeline →
                  </Link>
                </div>
                <p className="text-xs text-[#4A1724]">Rahul Sharma (Oct 19 launch target) vs Sarah Jenkins (Security compliance audit dependency).</p>
              </>
            ) : (
              <div className="text-center py-6">
                <AlertOctagon className="w-8 h-8 text-emerald-600 mx-auto mb-2" />
                <h3 className="font-bold text-sm text-[#4A1724]">Zero Active Speaker Conflicts Detected in Live Database</h3>
                <p className="text-xs text-[#96546A] mt-1">Real-time divergence analysis engine is monitoring incoming audio streams.</p>
              </div>
            )}
          </div>
        )}
